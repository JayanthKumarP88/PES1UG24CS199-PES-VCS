...
int index_load(Index *index) {
    ...
    while (fscanf(f, "%o %127s %llu %u %511s", &mode, hex, &mtime, &size, path) == 5) {
        IndexEntry *entry = &index->entries[index->count];
        entry->mode = mode;
        hex_to_hash(hex, &entry->hash); // Using 'hash' as defined in index.h
...
int index_save(const Index *index) {
    ...
    for (int i = 0; i < index->count; i++) {
        ...
        char hex[128];
        hash_to_hex(&entry->hash, hex); // Using 'hash'
        fprintf(f, "%06o %s %llu %u %s\n", 
                entry->mode, hex, (unsigned long long)entry->mtime_sec, 
                (unsigned int)entry->size, entry->path);
    }
...
int index_add(Index *index, const char *path) {
    ...
    ObjectID obj_hash;
    if (object_write(OBJ_BLOB, data, st.st_size, &obj_hash) < 0) {
        if (data) free(data);
        return -1;
    }
    if (data) free(data);
    ...
    entry->hash = obj_hash; // Using 'hash'
