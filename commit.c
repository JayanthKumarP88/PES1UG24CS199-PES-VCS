...
int commit_create(const char *message, ObjectID *commit_id_out) {
    ...
    // 1. Build a tree from the current index (staging area)
    if (tree_from_index(&c.tree) < 0) return -1;

    // 2. Read the current HEAD to establish parent history
    ...
    if (!author_str) author_str = "PES VCS User";
    snprintf(c.author, sizeof(c.author), "%s", author_str);
    
    c.timestamp = (uint64_t)time(NULL);

    // 4. Attach the commit message
    snprintf(c.message, sizeof(c.message), "%s", message);

    // 5. Serialize the Commit struct into binary text format
    void *data = NULL;
    size_t len = 0;
    if (commit_serialize(&c, &data, &len) < 0) return -1;

    // 6. Write the commit to the object database
    ...
    // 7. Update the HEAD pointer to point to this new commit
